...
class MIDITrack {
  constructor(type, id, parent) {
    ...
    this.beatDuration = function () {
      return ((60 / window.BPM) * 1000) / this.speedMultiplier;
    };
    ...
    document.addEventListener("rewindEvent", (e) => {
      this.rewind();
    });
    document.addEventListener("pauseEvent", (e) => {
      this.stopNote();
    });
  }
  ...
  update() {
    ...
    if (
      this.selectedBeats.length > 0 &&
      window.arrowLeft != window.arrowRight
    ) {
    ...
    if (window.isPlaying) {
      ...
      if (beatWasPlayed) {
        ...
        for (let i = 0; i < this.beats.length; i++)
          this.beats[i].elem.className = this.beats[i].elem.className.replace(
            " active",
            ""
          );
  ...
  updateNote() {
    ...
    let MIDINote = beat.hasOwnProperty("MIDINote")
      ? parseInt(beat.MIDINote)
      : -1;
    let noteDuration = beat.hasOwnProperty("duration")
      ? parseInt(beat.duration)
      : 0;
    let beatWasPlayed = false;
    ...
    if (beatWasPlayed) {
      ...
      this.CCUpdateTimeout = setTimeout(function () {
        ...
        if (v > -1) {
          ...
          self.currentCCGlide = beat.hasOwnProperty("CCGlide")
            ? parseFloat(beat.CCGlide)
            : 1;
        }
  ...
  createController() {
    ...
    but = TextInput("SPEED", this.speedMultiplier, (value) => {
      this.setSpeedMultiplier(value);
    });
    div.appendChild(but);

    but = TextInput("OUT", this.outputNum, (value) => {
      this.setOutputNum(value);
    });
    div.appendChild(but);

    but = TextInput("CHAN", this.outputChannel, (value) => {
      this.setOutputChannel(value);
    });
    div.appendChild(but);

    if (this.type === "cc") {
      but = TextInput("CC NUM", this.CCNum, (value) => {
  ...
  createEditor() {
    ...
    if (this.type === "note") {
      ...
      but = TextInput("DUR", duration, (value) => {
        this.setNoteDuration(value);
      });
      div.appendChild(but);
      ...
      but = TextInput(
        "CC VAL",
        beat !== null && beat.hasOwnProperty("CCValue") && beat.CCValue > -1
          ? beat.CCValue
          : "-",
        (value) => {
          this.setCCValue(value);
        }
      );
      div.appendChild(but);
      ...
      but = TextInput(
        "GLIDE",
        beat != null && beat.hasOwnProperty("CCGlide") ? beat.CCGlide : "100",
        (value) => {
          this.setCCGlide(value);
        }
      );
      div.appendChild(but);
    }

    but = Button("", "CLR", "click", (e) => {
      this.clearBeat(e);
    });
    div.appendChild(but);

    but = Button("", "ADD", "click", (e) => {
      this.addBeat(e);
    });
    div.appendChild(but);

    but = Button("", "DEL", "click", (e) => {
      this.removeBeat(e);
    });
    div.appendChild(but);
  }
  ...
  createConsole() {
    ...
    if (this.type === "note") {
      ...
      div.innerHTML =
        this.currentCCValue < 0 ? "" : this.currentCCValue.toFixed(0);
    }
  }
  ...
  setNoteDuration(value) {
    ...
    if (value < 0 && this.pendingNoteDuration < window.minNoteDuration)
      this.pendingNoteDuration = window.minNoteDuration;
    if (this.pendingNoteDuration < window.minNoteDuration)
      this.pendingNoteDuration = window.minNoteDuration;
  ...
  selectBeat(event, num) {
    ...
    this.pendingOctave =
      beat !== null && beat.hasOwnProperty("octave") ? beat.octave : -1;
    this.pendingNote =
      beat !== null && beat.hasOwnProperty("note") ? beat.note : -1;
    this.pendingNoteDuration =
      beat !== null && beat.hasOwnProperty("duration") ? beat.duration : 0;
    this.pendingCCValue =
      beat !== null && beat.hasOwnProperty("CCValue") ? beat.CCValue : -1;
  ...
  scrollNotesAndOctaves() {
    ...
    if (!self.scrollNotesAndOctavesInterval) {
      scroll();
      self.scrollNotesAndOctavesInterval = setInterval(
        scroll,
        self.scrollSpeed
      );
    }
